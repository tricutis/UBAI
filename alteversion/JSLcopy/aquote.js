...
// 2: zitate von Bert Hellinger bzw avurveda
...
//12: deutsche weisheiten (31 stuck)
//13: avurveda zitate
...
var greyfont="font-size:120%;width:80%;"
var quote_textname="quote_text"+quote_vers;
...
if (fun==4) {nextfun=0;     nextappear=0;span.style.cssText = "background-color:lightgrey;"+greyfont+cursor ; }
...
if (kind == 0) r="";
if (kind == 9)  { var temp=random_int(1,9) // deutsche: 2 3 4 7 12
                 switch (temp)
                 {
                  case 1: kind=1;break; 
                  case 2: kind=2;break;   
                  ...
                  case 6: kind=6 ;break;   
                  case 7: kind=7 ;bre                  
                  case 8: kind=12;break;   
                  case 9: kind=13;break;   
...
if (kind ==11) { var temp=random_int(1,6) // deutsche: 2 3 4 7 12
                 switch (temp)
                 {
                  case 1: kind=2;break; 
                  case 2: kind=3;break;   
                  case 3: kind=4;break;   
                  case 4: kind=7;break;   
                  case 5: kind=12;break;   
                  case 6: kind=13;break;   
                  default:kind=2
                  }
                }
...
var alle = "??"
+"$ Die Kraft, die ein Freund im anderen erweckt, ist stärker als die Kraft, die er selbst besitzt (M.Tau)"
...
+"$Satz der steigenden Entropie, vereinfacht: Um etwas sauberzumachen, muß etwas anderes dreckig werden. (Aber Du kannst alles dreckig machen, ohne etwas sauber zu bekommen.)"
...
+"$Wie soll ich wissen was ich denke bevor ich höre was ich sage? (Sponti-Sprüche)"
...
+"$Wahre menschliche Kultur gibt es erst, wenn nicht nur Menschenfresserei, sondern jede Art des Fleischgenusses als Kannibalismus gilt! (Wilhelm Busch /1832-1908/) "
;
...
+"<br>so fällt der Zufall weg; das Wesen, das besteht."
+"<br>Viel haben macht nicht reich. Der ist ein reicher Mann,"
+"<br>der alles was er hat, ohn' Leid verlieren kann."
+"<br>Freund, so du etwas bist, so bleib doch ja nicht stehn:"
+"<br>Man muß aus einem Lichte fort ins and're gehn."
+"<br>        Angelus Silesius"
+"$What, if you slept,"
...
+"<br>A new road or a secret gate,"
+"<br>And though I oft have passed them by,"
+"<br>A day will come at last when I "
+"<br>shall take the hidden paths that run"
+"<br>West of the Moon, East of the Sun"
+"$One ship sails east and one sails west"
+"<br>With the selfsame winds that blow"
+"<br>And it's the set of the sails "
+"<br>And not the gales"
+"<br>Which governs the ways to go"
...
function random_weisheit() {
var alle=
 "$Mit nichts bist du gekommen - mit nichts wirst du gehen."
+ "$Menschenfeinde sind oft nur besonders verletzlich."
+ "$Einer, der nur nach Macht und Einfluss strebt, muss irgendwann bitten."
+ "$Wer ehrlich ist, hat viele Feinde."
+ "$Für alles, was du gegeben hast oder was dir genommen wurde, erhältst du etwas zurück."
+ "$Nur weil sich Frauen und Männer nicht wirklich begreifen, können sie sich lieben."
+ "$Der Ehrgeiz des Einzelnen ist ein Vampir der Nachgiebigkeit vieler anderer."
+ "$Wer über Leichen geht, soll sich über Gestank nicht wundern."
+ "$Lass dich nicht von zuviel Freundlichkeit täuschen - oft sind die Raubeine wahrhaftiger."
+ "$Du entscheidest in jedem Augenblick neu, wer du bist."
+ "$Wer Arbeit sät, wird Mühsal ernten."
+ "$Wahr ist, was funktioniert. Aber falsch ist, was nur im Moment funktioniert."
+ "$Was hat für dich eine Bedeutung und warum?"
+ "$Jeder Mensch ist gleich interessant. Nur die Betrachtung ist verschieden."
+ "$Damit etwas oder jemand für dich wichtig ist, musst du Unwichtiges kennengelernt haben."
+ "$Für alles, was du einem anderen genommen hast, wirst du etwas geben müssen. "
+ "$Grosse Bequemlichkeit ist eine Art Warten auf das Ende."
+ "$Spürst du Aggressionen aufkommen, kauf dir einen Punchingball."
+ "$Eine Niederlage schmerzt nur, wenn man keine andere Option offen hat. "
+ "$Wer dich schlecht oder ungerecht behandelt, ist oft nur ein Werkzeug der Götter. "
+ "$Oft wird gerade von den grössten Ignoranten das grösste Verständnis geheuchelt. "
+ "$Frage dich nach jeder Beleidigung, ob nicht ein Körnchen Wahrheit darin war."
+ "$Wer einen Plan stur und zielstrebig verfolgt, übersieht meistens die Blumen am Wegrand."
+ "$Solange sich Arroganz nicht als Ignoranz und Intoleranz äussert, ist sie als Abgrenzungsmittel erlaubt. "
+ "$Oft verwechselt man Liebesfähigkeit mit der Sehnsucht, geliebt zu werden."
+ "$Wer sich einem anderen ganz öffnet, muss sich nicht wundern, wenn das Gegebene verwendet wird. "
+ "$Plane nicht, sondern nimm, was dir der Tag bringt."
+ "$Wer andere täuscht oder ein Doppelleben führt, wird irgendwann sehr enttäuscht werden."
+ "$Da erst die Zeit uns lebendig macht, sollten wir uns ernsthaft mit ihr auseinandersetzen."
+ "$Plus und Minus ergibt Minus - aber Doppelminus ergibt Plus."
+ "$Weil Magie im Grunde ganz einfach ist, gibt es Probleme in der Welt - weil Magie aber im Grunde sehr schwierig ist, gibt es sie auch.";
...
function quoteay() {
var alle = "?"
  + '$Nicht für deine eigene Person, nicht, um dir einen irdischen Wunsch zu erfüllen, sondern allein zum Wohle der Menschheit sollst du Krankheiten behandeln und dich damit vor allen anderen aus-zeichnen. '
   +' (Caraka)'
+'$Ist ein Arzt, mag er noch so gelehrt sein, unfähig, in die Seele des Patienten zu schauen, so ist er nicht berechtigt, die Behandlung von Kranken durchzuführen.'
+ ' (Caraka)'
+'$Der Ayurveda hat der Welt eine Botschaft zu verkünden; und wenn wir diese Botschaft nicht verstehen, dann wird die Welt zerbrechen.'
+ ' (Henry Sigerist,einer der bedeutendsten Experten für historische Medizin)'
+ '$Die Abhyanga (Ölmassage und Bad) sollte täglich ausgeführt werden. Sie vertreibt Alter, Anspannung und Ansammlungen von ~Vata~. Sie schenkt gute Sehfähigkeit, Ernährung für den Körper, langes Leben, guten Schlaf, gute und gesunde Haut. Sie sollte besonders ausgeführt werden am Kopf, den Ohren und Füßen.'
+ ' (Vagbath,Ashtanga Hrdaya,Dinacarya Adhyaya)'
+ '$Alle menschlichen Aktivitäten dienen dem Glück aller lebenden Wesen. Dieses Glück basiert auf Dharma (Rechtschaffenheit, richtige moralische Lebensführung), so sollte jede Person immer der Rechtschaffenheit folgen.'
+ ' (Vagbath,Ashtanga Hrdaya,Dinacarya Adhyaya)'
+'$Freunden sollte durch Gefühl und gute Taten gedient werden. Andere dagegen (Feinde, böse Menschen) sollten auf Abstand gehalten werden. '
+ ' (Vagbath,Ashtanga Hrdaya,Dinacarya Adhyaya)'
+'$Udvartana (Massage des Körpers mit einem weichen duftenden Pulver) besänftigt Kapha, verflüssigt das Fett, erzeugt Stabilität (Stärke, Kompaktheit) der Körperteile und macht eine exzellente Haut.'
+ ' (Vagbath,Ashtanga Hrdaya,Dinacarya Adhyaya)'
+'$